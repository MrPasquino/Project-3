...
ggpairs(data=house[,c('price','bedrooms','bathrooms','sqft_living', 'sqft_lot','sqft_above', 'sqft_basement')], title = 'Comparison Graph 1', aes(colour = as.factor(house$waterfront)))
...
```{r}
train.culled <- train[, !names(train) %in% c('trans.grade','f.bdrm.less.eq.8')]

model.length <- length(lm(price ~ . + f.waterfront + f.floor + f.cond + f.renov + f.zipcode, data = train.culled)$coef) -1

library("leaps")

model.forward <- regsubsets(price ~ . + f.waterfront + f.floor + f.cond + f.renov + f.zipcode, data = train.culled, method = "forward", nvmax = model.length)

model.backward <- regsubsets(price ~ . + f.waterfront + f.floor + f.cond + f.renov + f.zipcode, data = train.culled, method = "backward", nvmax = model.length)
...
```
Looking at the plots of the Bic and Adjusted R-squared values of 
...
par(mfrow=c(2,4))
plot(summary(model.forward)$bic)
plot(summary(model.forward)$adjr2)
plot(summary(model.backward)$bic)
plot(summary(model.backward)$adjr2)
plot(summary(model.forward.full)$bic)
plot(summary(model.forward.full)$adjr2)
plot(summary(model.backward.full)$bic)
plot(summary(model.backward.full)$adjr2)
```
