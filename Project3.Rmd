...
Digitally signed: James Armstrong
...
library(ggplot2)
library(GGally)
ggpairs(data=house[,c('price','bedrooms','bathrooms','sqft_living', 'sqft_lot','sqft_above', 'sqft_basement')], title = 'Comparison Graph 1', aes(colour = as.factor(house$waterfront))) + theme(axis.text.x = element_text(angle= 90, hjust= 1))
ggpairs(data=house[,c('price','bedrooms','bathrooms','sqft_living','floors', 'waterfront','condition')], title = 'Comparison Graph 2', aes(colour = as.factor(house$waterfront))) + theme(axis.text.x = element_text(angle= 90, hjust= 1))
ggpairs(data=house[,c('price','bedrooms','bathrooms','sqft_living','grade','yr_built','yr_renovated')], title = 'Comparison Graph 3', aes(colour = as.factor(house$waterfront))) + theme(axis.text.x = element_text(angle= 90, hjust= 1))
ggpairs(data=house[,c('price','sqft_lot','sqft_above', 'sqft_basement','floors', 'waterfront','condition')], title = 'Comparison Graph 4', aes(colour = as.factor(house$waterfront))) + theme(axis.text.x = element_text(angle= 90, hjust= 1))
ggpairs(data=house[,c('price','sqft_lot','sqft_above', 'sqft_basement','grade','yr_built','yr_renovated')], title = 'Comparison Graph 5', aes(colour = as.factor(house$waterfront))) + theme(axis.text.x = element_text(angle= 90, hjust= 1))
ggpairs(data=house[,c('price','floors', 'waterfront','condition','grade','yr_built','yr_renovated')], title = 'Comparison Graph 6', aes(colour = as.factor(house$waterfront))) + theme(axis.text.x = element_text(angle= 90, hjust= 1))
ggpairs(data=house[,c("sqft_living", "sqft_lot","sqft_above", "sqft_basement")],title = "Comparisons of Sqft Variables", aes(colour = as.factor(house$waterfront))) + theme(axis.text.x = element_text(angle= 90, hjust= 1))
```
...
summary(lm(log.price~log.grade-1))$coefficients[1]
```

The best value for 'b' should be 6.2438.
...
```{r}
model.length = length(lm(price ~ . + f.waterfront + f.floor + f.cond + f.renov + f.zipcode - f.bdrm.less.eq.8 - trans.grade, data = train)$coef) - 1
...
ggplot(df.models[df.models$model.type=="Backward Model Bic" |df.models$model.type=="Forward Model Bic",]) + geom_point(aes(x=x,y=y, color=model.type, alpha = 0.4))
ggplot(df.models[df.models$model.type=="Backward Model AdjR2" |df.models$model.type=="Forward Model AdjR2",]) + geom_point(aes(x=x,y=y, color=model.type, alpha = 0.4))
```
Looking at the summaries and the plot of the summaries for the bic and adjusted R^2 values of the backwards and forwards models, these two methods produce similar results in terms of bic and R^2 values.
